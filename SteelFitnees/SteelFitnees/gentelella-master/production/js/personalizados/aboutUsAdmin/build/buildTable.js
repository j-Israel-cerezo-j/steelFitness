function buildTable(json, jsonOnkeyp = true) {
    document.getElementById("containerTableAboutUs").innerHTML = "";
    var ban = false;
    var cont = 1;
    var html = `           
                  <div>                   
                      ...
                      <table class="table table-striped jambo_table bulk_action" id="tbl-roles">
                        <thead>
                          <tr class="headings">
                            ...
                            <th class="column-title" style="display: table-cell;">Misión</th>                            
                            <th class="column-title" style="display: table-cell;">Visión</th>
                            <th class="column-title" style="display: table-cell;">Valores</th>
        ...
        for (var i = 0; i < json.length; i++) {
            ...
            html +=
                `<tr class="even pointer">
                                    ...
                                    <td class=" ">${cont++}</td>`
            html += ` <td class=" ">${json[i].mision}</td>`
            html += ` <td class=" ">${json[i].vision}</td>`
            html += ` <td class=" ">${json[i].valores}</td>`
    ...
    if (ban) {
        document.getElementById("containerTableAboutUs").innerHTML = html;
        initChecksTable();
        addEventCheckAll();
    } else if (!jsonOnkeyp) {
        document.getElementById("containerTableAboutUs").innerHTML = "<h4 class='text-center'>No se encontro ninguna coincidencia</h4>"
    } else {
        document.getElementById("containerTableAboutUs").innerHTML = "<h4 class='text-center'>Aún no hay registros agregados</h4>"
    }
