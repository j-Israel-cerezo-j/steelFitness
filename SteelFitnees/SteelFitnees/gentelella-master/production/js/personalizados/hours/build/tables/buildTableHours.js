function buildTableHours(json, jsonOnkeyp = true) {
        ...
        for (var i = 0; i < json.length; i++) {
            ...
            html +=
                `<tr class="even pointer">
                                    <td class="a-center ">
                                        <div class="icheckbox_flat-green" style="position: relative;">
                                            <input type="checkbox" value="${json[i].idHorario}" id="cheksa${json[i].idHorario}" class="flat" name="table_records" style="position: absolute; opacity: 0;">
                                        </div>
                                    </td>
                                    <td class=" ">${cont++}</td>`
    ...
    if (ban) {
        document.getElementById("containerTableHours").innerHTML = html;
        initChecksTable();
        addEventCheckAll();
    } else if (!jsonOnkeyp) {
        document.getElementById("containerTableHours").innerHTML = "<h4 class='text-center'>No se encontro ninguna coincidencia</h4>"
    } else {
        document.getElementById("containerTableHours").innerHTML = "<h4 class='text-center'>Aún no hay registros agregados</h4>"
    }
