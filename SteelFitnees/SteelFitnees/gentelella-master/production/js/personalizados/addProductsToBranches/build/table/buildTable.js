function buildTable(json, jsonOnkeyp = true) {
    document.getElementById("containerTableProductBranches").innerHTML = "";
    var ban = false;
    var cont = 1;
    var html = `           
                  <div class="x_title">
                    <h2>Productos en sucursales<small></small></h2>
                      ...
                      <table class="table table-striped jambo_table bulk_action" id="tbl-roles">
                        <thead>
                          <tr class="headings">
                            ...
                            <th class="column-title" style="display: table-cell;">Descripción de la sucursal</th>
                            <th class="column-title" style="display: table-cell;">Producto</th>
                            <th class="column-title" style="display: table-cell;">Descripción del producto</th>
                            <th class="column-title" style="display: table-cell;">Cantidad</th>
                            <th class="column-title" style="display: table-cell;">Precio(c/u)</th>
                            <th class="column-title" style="display: table-cell;">Imagen del producto</th>
        ...
        for (var i = 0; i < json.length; i++) {
            ...
            html +=
                `<tr class="even pointer">
                                    <td class="a-center ">
                                        <div class="icheckbox_flat-green" style="position: relative;">
                                            <input type="checkbox" value="${json[i].idRegistro}" id="cheksa${json[i].idRegistro}" class="flat" name="table_records" style="position: absolute; opacity: 0;">
                                        </div>
                                    </td>
                                    <td class=" ">${cont++}</td>`
            html += ` <td class=" ">${json[i].nombreSuc}</td>`
            html += ` <td class=" ">${json[i].descriptBranche}</td>`
    ...
    if (ban) {
        document.getElementById("containerTableProductBranches").innerHTML = html;
        initChecksTable();
        addEventCheckAll();
    } else if (!jsonOnkeyp) {
        document.getElementById("containerTableProductBranches").innerHTML = "<h4 class='text-center'>No se encontro ninguna coincidencia</h4>"
    } else {
        document.getElementById("containerTableProductBranches").innerHTML = "<h4 class='text-center'>Aún no hay registros agregados</h4>"
    }
